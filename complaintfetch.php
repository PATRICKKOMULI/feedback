...
if (isset($_POST['search'])) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `tbl_complaint` WHERE date(`time`) BETWEEN '$date1' AND '$date2'") or die(mysqli_error());
...
?>
    ...
    $query = mysqli_query($conn, "SELECT * FROM `tbl_complaint`") or die(mysqli_error());
    while ($fetch = mysqli_fetch_array($query)) {
        ?>
        <tr>
            <td><?php echo $fetch['Date'] ?></td>
